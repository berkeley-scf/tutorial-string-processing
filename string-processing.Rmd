...
The analogue of *regexpr* vs. *gregexpr* and *sub*
...
Please look at Section 3 of [our tutorial on using the bash shell](http://statistics.berkeley.edu/computing/training/tutorials) to learn the regular expression syntax that we'll use in the remainder of this tutorial. For other resources, Duncan Temple Lang (UC Davis Statistics)
...
First let's see the use of character sets and characters classes.
...
Here's a search for US/Canadian/Caribbean phone numbers: 
```{r, repetitions}
```
...
And reference back to a detected pattern when doing a replacement.
...
## 3.2) Using regex in Python

### 3.2.1) Working with patterns
...
For R, some useful packages are *httr* and *RCurl* are good for interacting with webpages and *XML* for reading/parsing HTML and XML files. *jsonlite* is good for reading/parsing JSON. See the [WebTechnologies CRAN task view](https://cran.r-project.org/web/views/WebTechnologies.html) for a summary.
